...
type Params = {
  ...
  atrLen5: number;
  atrLen1: number;
  minBoxAtrMult: number;
  maxBoxAtrMult: number;

  breakBufferAtr5: number;
  stopBufferAtr5: number;

  starterBandPct: number;
...
/**
 * Portal tooltip: fixed positioning, never clipped by overflow/stacking.
 */
function InfoTip({ text }: { text: string }) {
  ...
  function recalc() {
    ...
    // Prefer right of icon; if overflow, shift left
    let left = r.left + 22;
    left = Math.min(left, window.innerWidth - estimatedW - padding);
    left = clamp(left, padding, window.innerWidth - estimatedW - padding);

    // Prefer below icon; if too low, show above
    let top = r.bottom + 10;
    const estH = 80; // approx; tooltip can grow, but this is fine for placement
  ...
  async function runBacktest() {
    ...
    try {
      const res = await fetch("/api/backtest", {
        ...
        body: JSON.stringify({ candles1m, strategy: "mgc_accum_breakout", params }),
      });

      const data = (await res.json()) as BacktestResponse;
      if (!res.ok) throw new Error((data as any)?.error ?? "Backtest failed");
      setResult(data);
    } catch (e: any) {
          ...
          <div style={cardInnerStyle}>
            <div style={topControlsGrid}>
              ...
              <div style={runBlockStyle}>
                <div>
                  <div style={labelTitleStyle}>Run</div>
                  <div style={labelSubStyle}>
                    Risk: <code style={codeStyle}>${params.riskPerCampaign}</code> per trade • Max trades/day:{" "}
                    <code style={codeStyle}>{params.maxCampaignsPerDay}</code>
                  </div>
                </div>

                <button
                  onClick={runBacktest}
                  disabled={!candles1m?.length || loading}
                  style={runBtnStyle(!candles1m?.length || loading)}
                >
                  {loading ? "Running..." : "Run Backtest"}
                </button>

                <div style={{ fontSize: 12, opacity: 0.68, marginTop: 10, lineHeight: 1.5 }}>
                  Adjust Parameters → Run again. Filters only affect the table.
                </div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    ...
                    <>
                      <div style={kpiGrid}>
                        <StatCard
                          ...
                          accent={summary!.netPnl > 0 ? "good" : summary!.netPnl < 0 ? "bad" : "neutral"}
                          hint="Total after all trades"
                        />
                        <StatCard
                          label="Max Drawdown"
                          value={formatMoney(summary!.maxDd)}
                          accent={summary!.maxDd < 0 ? "bad" : "neutral"}
                          hint="From equity curve"
                        />
                        <StatCard label="Win Rate" value={formatPct(summary!.winRate)} hint="wins / trades" />
                        <StatCard label="Profit Factor" value={(summary!.profitFactor ?? 0).toFixed(2)} hint="gross profit / loss" />
                        <StatCard label="Trades" value={String(summary!.trades)} hint={`Boxes: ${summary!.boxesFound ?? 0}`} />
                        <StatCard label="Bars" value={`${summary!.candles1m ?? 0} (1m)`} hint={`${summary!.candles5m ?? 0} (5m)`} />
                      </div>
                      ...
                      <div style={chartPanelStyle}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                          ...
                          <div style={{ fontSize: 12, opacity: 0.7 }}>
                            {result.equity?.length ? `${result.equity.length} points` : "No equity points yet"}
                          </div>
                        </div>
                ...
                <div style={panelStyle}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    ...
                    {result ? (
                      <div style={{ fontSize: 12, opacity: 0.7 }}>
                        Showing {Math.min(filteredTrades.length, 200)} / {filteredTrades.length} (filtered)
                        {" • "}
                        Filtered Net: <b>{formatMoney(filteredStats.net)}</b>
                        {" • "}
                        Filtered WinRate: <b>{formatPct(filteredStats.winRate)}</b>
                      </div>
                    ) : null}
                  </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div style={panelStyle}>
                  ...
                  <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    ...
                    <button
                      type="button"
                      onClick={() => setFilters((f) => ({ ...f, direction: "ALL", outcome: "ALL", tagQuery: "", minPnl: "", maxPnl: "" }))}
...
const cardTopGlowStyle: React.CSSProperties = {
  height: 2,
...
const uploadBlockStyle: React.CSSProperties = {
  borderRadius: 18,
...
const fileBtnStyle: React.CSSProperties = {
  ...
  borderRadius: 14,
  border: "1px solid rgba(57,255,20,0.35)",
  background: "linear-gradient(135deg, rgba(57,255,20,0.22), rgba(0,255,170,0.10))",
  color: "#EAFBEA",
  fontWeight: 800,
  cursor: "pointer",
...
const tooltipFixedStyle: React.CSSProperties = {
  position: "fixed",
  zIndex: 99999,
...
function runBtnStyle(disabled: boolean): React.CSSProperties {
  return {
    ...
    background: disabled
      ...
      : "linear-gradient(135deg, rgba(57,255,20,0.35), rgba(0,255,170,0.18))",
